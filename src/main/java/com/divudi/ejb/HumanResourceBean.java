...
//                
                ...
                System.out.println("1.staffLeave.size() = " + staffLeave.size());
                ...
                if (staffLeave.size() > 1 && ss.getShift() != null) {
                    ...
                    for (StaffLeave sl : staffLeave) {
                        if (sl.getLeaveType() != LeaveType.No_Pay_Half) {
                            ...
                            System.out.println("1.sl.getLeaveType() = " + sl.getLeaveType());
                        }
                    }
//                        dbl += (ss.getShift().getDurationMin() * 60);
...
//            //System.out.println("ss.getShift().getName(); = " + ss.getShift().getName());
    ...
    public boolean checkDateAnalsed(Date date, Staff staff) {
        ...
        HashMap hm = new HashMap();
        hm.put("d", date);
        hm.put("dts", Arrays.asList(new DayType[]{DayType.DayOff, DayType.Poya, DayType.MurchantileHoliday, DayType.SleepingDay,DayType.PublicHoliday}));
        hm.put("stf", staff);
